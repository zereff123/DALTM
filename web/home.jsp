...
<head>
    ...
    <style>
        ...
        h1, h2, h3, h4, h5, .navbar-brand, .hero-title, .btn { font-family: 'Montserrat', sans-serif; letter-spacing: -0.5px; }
        ...
        .hero-section { background: url('https://images.unsplash.com/photo-1504214208698-ea1916a2195a?ixlib=rb-4.0.3&auto=format&fit=crop&w=1920&q=80') no-repeat center center; background-size: cover; height: 400px; position: relative; border-radius: 0 0 50px 50px; margin-bottom: 40px; display: flex; align-items: center; justify-content: center; color: white; text-align: center; }
        .hero-overlay { position: absolute; top: 0; left: 0; width: 100%; height: 100%; background: linear-gradient(to bottom, rgba(0,0,0,0.3), rgba(0,0,0,0.6)); border-radius: 0 0 50px 50px; }
        ...
        .price-tag { position: absolute; bottom: 10px; right: 10px; background: rgba(0,0,0,0.8); backdrop-filter: blur(5px); color: #fff; padding: 5px 15px; border-radius: 20px; font-weight: 700; font-size: 1.1rem; font-family: 'Montserrat', sans-serif; }
...
<body>
            ...
            <div class="collapse navbar-collapse" id="navbarNav">
                <ul class="navbar-nav ms-auto align-items-center">
                    ...
                    <c:if test="${sessionScope.account != null}">
                        ...
                        <c:if test="${sessionScope.account.role == 'GUIDE'}">
                            <li class="nav-item me-2">
                                <a class="btn btn-outline-light rounded-pill btn-sm" href="guide"><i class="fas fa-map-signs"></i> Guide Portal</a>
                            </li>
                        </c:if>
                ...
                <div class="col-lg-4 col-md-6 mb-4">
                    <div class="card tour-card h-100 position-relative">
                        ...
                        <div class="card-img-wrapper">
                            ...
                            <a href="detail?id=${o.id}" class="stretched-link"></a>
                        </div>
                        ...
                        <div class="card-body d-flex flex-column">
                            ...
                            <div class="d-flex justify-content-between text-muted small mb-2">
                                <span><i class="fas fa-map-marker-alt text-danger me-1"></i> ${o.location}</span>
                                
                                <span class="fw-bold text-primary">
                                    <c:choose>
                                        <c:when test="${o.transport == 'Máy Bay'}"><i class="fas fa-plane me-1"></i></c:when>
                                        <c:when test="${o.transport == 'Tàu Hỏa'}"><i class="fas fa-subway me-1"></i></c:when>
                                        <c:when test="${o.transport == 'Du Thuyền'}"><i class="fas fa-ship me-1"></i></c:when>
                                        <c:otherwise><i class="fas fa-bus me-1"></i></c:otherwise>
                                    </c:choose>
                                    ${o.transport}
                                </span>
                            </div>
                        ...
                        <div class="card-footer bg-white border-0 pb-3 pt-0" style="position: relative; z-index: 2;">
                            <c:if test="${sessionScope.account == null}">
                            ...
                            <c:if test="${sessionScope.account != null}">
                                <c:if test="${o.currentCapacity < o.maxCapacity}">
                                    <a href="pre-booking?id=${o.id}" class="btn btn-booking btn-custom w-100 shadow-sm text-decoration-none text-center d-block pt-2">
                                        <i class="fas fa-file-invoice-dollar me-1"></i> Tiến Hành Đặt Vé
                                    </a>
                                </c:if>
    ...
    <script>
        ...
        socket.onopen = function(event) { console.log("WebSocket Home Connected!"); };
